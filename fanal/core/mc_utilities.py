...
def particle_mc_info(particle_dict : Mapping[int, MCParticle],
	...
	for indx, part in particle_dict.items():
		...
		print(part_tab + 'Prod.: Mom = ({0:.1f}, {1:.1f}, {2:.1f}) keV,   KinE: {3:.1f} keV'
			             .format(part.p[0]/units.keV, part.p[1]/units.keV,
			             	     part.p[2]/units.keV, part.E/units.keV))
		...
		print(part_tab + 'Decay: Volume = {0},   Vertex = ({1:.1f}, {2:.1f}, {3:.1f}) mm'
			             .format(part.final_volume, part.final_vertex[0],
			             	     part.final_vertex[1], part.final_vertex[2]))
	...
	for iFileName in iFileNames:
		with tb.open_file(iFileName, mode='r') as h5in:
			...
			if event_id in extents_df['evt_number'].tolist():
				...
				mcEvtParticles  = load_mcparticles(iFileName, (event_index, event_index+1))
				evt_mcParticles = mcEvtParticles[event_id]
				mcEvtHits  = load_mchits(iFileName, (event_index, event_index+1))
				...
				particle_mc_info(evt_mcParticles, with_hits=with_hits,
