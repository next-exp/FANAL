...
def get_active_size(detname : DetName) -> VolumeDim:
    ...
    if detname == DetName.demo:
        return VolumeDim(z_min =   0.0  * units.mm,
                         z_max = 309.55 * units.mm,
                         rad   =  97.1  * units.mm)

    elif detname == DetName.new:
        return VolumeDim(z_min =   0.0 * units.mm,
                         z_max = 532.0 * units.mm,
                         rad   = 208.0 * units.mm)

    elif detname == DetName.next100:
        return VolumeDim(z_min =    0.0  * units.mm,
                         z_max = 1204.95 * units.mm,
                         rad   =  492.0  * units.mm)

    elif detname == DetName.flex100:
        return VolumeDim(z_min =    0.0  * units.mm,
                         z_max = 1204.95 * units.mm,
                         rad   =  492.0  * units.mm)

    elif detname == DetName.next100_alaHD:
        return VolumeDim(z_min = -650.0 * units.mm,
		                 z_max =  650.0 * units.mm,
						 rad   =  534.5 * units.mm)

    elif detname == DetName.next500:
        return VolumeDim(z_min = -1000.0 * units.mm,
		                 z_max =  1000.0 * units.mm,
						 rad   =  1000.0 * units.mm)
        ...
        return VolumeDim(z_min = -1500.0 * units.mm,
                         z_max =  1500.0 * units.mm,
						 rad   =  1500.0 * units.mm)

    elif detname == DetName.next_hd:
        return VolumeDim(z_min = -1300.0 * units.mm,
                         z_max =  1300.0 * units.mm,
						 rad   =  1300.0 * units.mm)
...
def is_detector_symmetric(detname : DetName) -> bool:
    ...
    if ((detname == DetName.next500)  or
        ...
        (detname == DetName.next100_alaHD) or
